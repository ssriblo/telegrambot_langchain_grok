...
def get_user_state(user_id: int) -> UserState:
    ...
    if state is None:
        ...
        log.debug(f"[STATE] Created new state for user {user_id}")
    return state
...
def update_stage(user_id: int, stage: str) -> None:
    ...
    log.info(f"[STATE] user={user_id} stage: {old} → {stage}")
...
def set_style(user_id: int, style: str) -> None:
    ...
    state.style = style
    save_user_state(state)
    log.info(f"[STATE] user={user_id} style: {old} → {style}")
...
def set_language(user_id: int, language: str) -> None:
    ...
    save_user_state(state)
    log.debug(f"[STATE] user={user_id} language set to {language}")
...
def set_raw_preferences(user_id: int, text: str) -> None:
    ...
    save_user_state(state)
    log.info(f"[STATE] user={user_id} preferences saved: {state.preferences.interests!r}")
...
def set_location(user_id: int, lat: float, lon: float) -> None:
    ...
    save_user_state(state)
    log.info(f"[STATE] user={user_id} location set to ({lat}, {lon})")
...
def set_program(user_id: int, program_id: str) -> None:
    ...
    if program_id in state.generated_programs:
        ...
        log.info(f"[STATE] user={user_id} program={program_id}, route={state.current_route}")
    else:
        log.warning(f"[STATE] user={user_id} program={program_id} NOT FOUND in generated_programs!")
...
def save_generated_programs(user_id: int, programs: Dict[str, List[str]]) -> None:
    ...
    save_user_state(state)
    log.info(f"[STATE] user={user_id} generated_programs saved: { {k: len(v) for k, v in programs.items()} }")
...
def mark_place_visited(user_id: int, place_id: str) -> None:
    ...
    save_user_state(state)
    log.debug(f"[STATE] user={user_id} visited: {state.visited_places}")
...
def reset_user_state(user_id: int) -> None:
    ...
    log.info(f"[STATE] user={user_id} state RESET")
